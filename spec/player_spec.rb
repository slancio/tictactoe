...
describe Player do
  ...
  describe '#move' do
    it 'raises an error when called' do
      expect { test_player.move }
        .to raise_error('Player::move should not be called')
      expect { test_player.move['fhqwhgads'] }
        .to raise_error('Player::move should not be called')
      expect { test_player.move(TicTacToe.new, :x) }
        .to raise_error('Player::move should not be called')
    end
  end
end
...
describe HumanPlayer do
  let(:test_human) { HumanPlayer.new('User') }
  ...
  #   let(:test_game) { TicTacToe.new(players: {x: test_human,
  #                                             o: HumanPlayer.new("Bob")})}
  ...
  #     expect { test_thread.run }.
  #       to output(/, please mark a space. Format: row,column./).to_stdout
  #     test_thread.kill
  #   end
  #
...
describe ComputerPlayer do
  ...
  describe '#move' do
    ...
    it 'blocks opponent winning moves' do
